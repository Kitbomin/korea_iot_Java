 ...
 * === Stream API의 최종연산 ===
 * 1. collect 메서드
 *  : 스트림의 요소를 변환하거나 집계하여 최종 결과를 생성하는데에 사용된다.
 *  - 스트림의 종결 작업 중 하나
 *  - 특정 컨테이너(List, Set, Map)으로 변환하거나 값을 합산, 집계하는데 사용
 *
 * 2. Collectors 클래스
 *  : 다양한  Collectors를 제공하여 collect() 메서드와 결합하여 하나의 결과를 반환
 *  - 다양한 정적 메서드 제공
 *
 *    1) to List, toSet, toMap: 해당 컬렉션으로 값을 반환
 *    2) joining(): 문자열 요소를 연결
 *    3) counting(): 스트림의 요소 수를 계산
 *    4) grouping(): 요소를 그룹화 하여 반환
...
public class C_Collect {
    public static void main(String[] args) {
        ...
        //Collectors.groupingBy(): 특정 기준에 따라 그룹화
        ...
        //Collectors.partitioningBy() : 조건에 따라 두 그룹으로 나눔(해당 조건이 참/거짓인 경우로 나눔) | filter 보다 데이터가 정교함
