...
public class BookRepository {
    ...
    public Book save (Book book) {
        //중복된 id 값(고유값)일 경우 도서 저장 X
        if (bookStore.containsKey(book.getId())) {

            //Map.containKey(Key타입 데이터) : 포함 여부를 boolean으로 반환
            System.out.println("[ " + book.getId() + " ] 의 책이 이미 존재합니다. 다시 입력해주셔용");
    ...
    public Book findById (String id) {
        return bookStore.get(id); //Map은 키값 활용도가 높음

        /*
         책 저장소 List 사용 시 책 저장소 전체를 순회해 현재 매개변수의 ID값과 일치하는 요소를 필터링 해야함
         근데 Map을 쓴다? -> 바로 접근 가능하다
        */
    }
    ...
    public boolean delete(String id) {
        if (!bookStore.containsKey(id)){
            //삭제 하고자 하는 id의 책이 없는 경우
            System.out.println("[ " + id + " ]의 책이 존재하지 않습니다. 책이 삭제되지 않았습ㄴ디ㅏ..");
            return false;
        }
        bookStore.remove(id);
        return true;
