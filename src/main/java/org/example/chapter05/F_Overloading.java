...
// === 메서드 오버로딩 === //
//: 중복 정의
//: 한 클래스 내에서 동일한 이름의 '메서드'를 여러개 정의함
...
//      >> 코드의 재사용성, 가독성 향상
//   - 컴파일 시 타입 체크가 강화됨
//      >> 올바른 매개변수를 가진 메서드를 자동 선택한다는 점

//3. 오버로딩의 단점
//   - 중복적으로 의미없이 생성하는 경우-> 실질적으로 필요한 구현 메서드 찾기에 어려움이 있다.(복잡성 증가)
//   -

class Calc {
    // 오버로딩의 성립조건: 메서드명 동일, 매개변수의 타입 || 개수 차이, 반환타입은 무관
    // 변수는 동일한 이름의 재정의가 불가능함

    int add(int a, int b) {
        return a+b;
    }

    //long add (int a, int b) {return a + b;} => add(int, int)가 이미 정의되어있어서 못씀
